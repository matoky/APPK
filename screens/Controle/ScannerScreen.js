...
export default function ScanCoupeScreen({ navigation }) {
    ...
    const sendScannedData = async () => {
        if (scannedDataList.length > 0) {
            ...
            const apiUrl = `${config.API_HOST}/qrControl`;
            ...
            try {
                ...
                // let doublonString = "";
                // if (response.data.doublons && Array.isArray(response.data.doublons)) {
                //     doublonString = response.data.doublons.map(d => `<div>${d}</div>`).join("<hr/>");
                // }

                // Swal.fire({
                //     title: 'Message',
                //     html: `<div>${response.data.message}</div>` + (doublonString ? `<hr/><div>${doublonString}</div>` : ""),
                //     icon: 'info',
                // });
